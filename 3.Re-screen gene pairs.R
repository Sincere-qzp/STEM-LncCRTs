...
PCC_IM <- apply(ssg2imm2, 1, PCOR_IM)
...
system.time({
  for (i in 1:1000) {   # 1000 times
    ...
    PCC_IM2 <- apply(ssg2imm2, 1, PCOR_IM)
    ...
    for (j in seq_len(nrow(WPC))) {
      if (WPC[j, 3] < WPC2[j, 3]) {
        ...
        permutation_LI[j, 3] <- permutation_LI[j, 3] + 0
      }
    }
    ### Permutation results are stored in permutation_LI
  }
})   # Timing finished

write.csv(permutation_LI,
